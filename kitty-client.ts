...
/** Translate kitty listen_on values that use abstract Unix sockets (`unix:@name` / `@name`). */
export function resolveUnixSocketPath(path: string): string {
	if (path.startsWith("@")) return `\0${path.slice(1)}`;
	return path;
}
...
export class KittyClient {
	...
	private async sendAndReceive(payload: Buffer): Promise<string> {
		...
		return new Promise<string>((resolve, reject) => {
			...
			socket.on("data", (chunk: Buffer) => {
				chunks.push(chunk);
				// Cheap gate: DCS responses end with the bytes 0x1b 0x5c (ESC backslash).
				// Only pay for the full concat + regex when the accumulated bytes actually
				// end with the suffix, so large get-text responses avoid O(N^2) string
				// allocations and GC pressure on every chunk.
				if (!endsWithDcsSuffix(chunks)) return;
				const raw = buffered();
			...
			// the extension runs under Bun. Write without ending;
	...
	private openSocket(): Promise<net.Socket> {
		...
		return new Promise<net.Socket>((resolve, reject) => {
			...
			socket.once("connect", () => {
				// Drop connect-phase error handling so later IO errors are owned by the caller.
 ...
 * Abstract Linux sockets use `unix:@name` (or bare `@name`) and connect to `"\0name"`.
 * IPv6 hosts in bracket form (`tcp:[::1]:12345`, `tcp6:[::1]:12345`) yield host `"::1"` without brackets.
 *
 * `fd:N` is intentionally rejected: this client opens a fresh connection per command and
 * cannot reuse a single inherited connected FD as a multi-command listening endpoint.
 */
export function parseListenOn(value: string): SocketTarget {
	if (value.startsWith("fd:")) {
		throw new KittyRemoteControlError(
			`Inherited fd:N remote-control sockets are not supported. Set KITTY_LISTEN_ON / kitty.listenOn to a reusable unix: or tcp: listening socket (got ${value}).`,
		);
	}
	if (value.startsWith("unix:")) {
		return { kind: "unix", path: resolveUnixSocketPath(value.slice("unix:".length)) };
	}
	...
	return { kind: "unix", path: resolveUnixSocketPath(value) };
}

/** True when accumulated chunks end with the DCS response suffix (bytes 0x1b 0x5c). */
function endsWithDcsSuffix(chunks: Buffer[]): boolean {
	if (chunks.length === 0) return false;
	const last = chunks[chunks.length - 1]!;
	const len = last.length;
	if (len === 0) return false;
	if (len >= 2) return last[len - 2] === 0x1b && last[len - 1] === 0x5c;
	// Single trailing byte: the 0x1b must be the final byte of the previous chunk.
	if (chunks.length < 2) return false;
	const prev = chunks[chunks.length - 2]!;
	return prev.length > 0 && prev[prev.length - 1] === 0x1b && last[0] === 0x5c;
}
