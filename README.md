...
When a subagent finishes work, use `interactive_shell({ sessionId: "...", outputLines: 200, outputMaxChars: 20000 })` to pull its output into the main agent context. Dispatch mode also sends a completion notification automatically. Prefer `outputLines` / `outputMaxChars` (and `incremental` / `drain` for long sessions) over any legacy transfer settings.
...
| ----------------------------- | --------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| `kitty.listenOn`              | `KITTY_LISTEN_ON`                                   | Kitty remote-control socket (`unix:…`, `unix:@name`, `tcp:…`, `tcp6:…`; `fd:N` is not supported) |
...
| `kitty.connectTimeoutMs`      | 5000                                                | Remote-control connect timeout                                                                   |
| `kitty.pollIntervalMs`        | 500                                                 | Output polling interval                                                                          |
| `kitty.killGraceMs`           | 5000                                                | Grace period after signal before force-closing a managed window                                  |
