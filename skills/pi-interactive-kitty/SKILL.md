...
| **Visibility**    | User sees kitty tab             | User sees kitty tab (or in managed kitty) | Hidden from user                      |
...
| **User control**  | User can type directly in kitty | User can type directly in kitty           | No intervention                       |
| **Best for**      | Long tasks needing supervision  | Fire-and-forget delegations               | Parallel tasks, structured delegation |

**Foreground subagents** run in a kitty tab where the user watches (and can intervene). Use `interactive_shell` with `mode: "hands-free"` to monitor while receiving periodic updates, or `mode: "dispatch"` to be notified on completion without polling.

**Dispatch subagents** also use `interactive_shell` but with `mode: "dispatch"`. The agent fires the session and moves on. When the session completes, the agent is woken up via `triggerTurn` with the output in context. Add `background: true` to avoid focusing the new kitty tab.

**Monitor mode** (`mode: "monitor"`) runs in managed kitty and event-driven. It wakes the agent on structured monitor trigger events (stream or poll-diff), so there is no polling loop.
...
Quoted prompt text plus `--hands-free` or `--dispatch` turns `/spawn` into a monitored delegated run instead of a plain interactive kitty tab:
...
The session still opens as a managed kitty tab, but does not steal focus. Multiple dispatches can run concurrently:
...
// → Kitty tab is created without focus. User can /attach to watch. Agent notified on completion.
```
...
The user sees the kitty tab immediately. You get control back to continue working, and the user can type directly in the tab when needed.
...
**Rate limited:** Queries are limited to once every 60 seconds. If you query too soon, the tool will automatically wait until the limit expires before returning. The user is watching the kitty tab in real-time - you're just checking in periodically.
...
The `reason` parameter is **UI-only** - it's shown in the kitty tab header but NOT passed to the subprocess.
...
When the subagent finishes, query or drain the session output directly:

```
[Subagent finishes work in kitty tab]
        ↓
[You call interactive_shell({ sessionId, outputLines: 200 })]
        ↓
...
**Configuration:** `outputLines` (default: 20, max: 200), `outputMaxChars` (default: 5KB, max: 50KB)
...
Write full transcript to `~/.pi/agent/cache/interactive-kitty/snapshot-*.log`:
...
// Start an event-driven monitor session (in managed kitty)
interactive_shell({
	command: "npm test --watch",
	mode: "monitor",
...
If a session is backgrounded during local testing, the agent should still clean it up later unless the user clearly wants it kept.
...
**Background dispatch (kitty tab created without focus):**
