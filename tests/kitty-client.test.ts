...
describe("parseListenOn", () => {
	...
	it("maps abstract unix sockets to null-prefixed paths", () => {
		expect(parseListenOn("unix:@kitty")).toEqual({ kind: "unix", path: "\0kitty" });
		expect(parseListenOn("@kitty")).toEqual({ kind: "unix", path: "\0kitty" });
	});

	it("rejects inherited fd:N sockets with an actionable error", () => {
		expect(() => parseListenOn("fd:3")).toThrow(/fd:N remote-control sockets are not supported/);
	});
...
describe("KittyClient response handling", () => {
	...
	it("destroys the socket when the peer ends without a complete DCS response", async () => {
		const { KittyClient } = await import("../kitty-client.js");
		const client = new KittyClient({ kitty: { responseTimeoutMs: 5000 } } as any);
		class FakeSocket extends EventEmitter {
			destroy = vi.fn();
	...
	it("destroys the socket when an error occurs while waiting for a response", async () => {
		const { KittyClient } = await import("../kitty-client.js");
		const client = new KittyClient({ kitty: { responseTimeoutMs: 5000 } } as any);
		class FakeSocket extends EventEmitter {
			destroy = vi.fn();
...
describe("KittyClient.openSocket connect lifecycle", () => {
	it("rejects with a timeout error when the connect never completes", async () => {
		const { KittyClient } = await import("../kitty-client.js");
		const client = new KittyClient({
			kitty: { listenOn: "unix:/tmp/pi-interactive-kitty-test.sock", connectTimeoutMs: 50 },
		} as any);
		...
		const spy = vi.spyOn(net.default, "createConnection").mockImplementation((() => socket as any) as typeof net.default.createConnection);
	...
	it("removes the connect-phase error listener after a successful connect", async () => {
		const { KittyClient } = await import("../kitty-client.js");
		const client = new KittyClient({
			kitty: { listenOn: "unix:/tmp/pi-interactive-kitty-test.sock", connectTimeoutMs: 5000 },
		} as any);
		...
		const socket = new FakeSocket();
		const net = await import("node:net");
		const spy = vi.spyOn(net.default, "createConnection").mockImplementation((() => {
			queueMicrotask(() => socket.emit("connect"));
			return socket as any;
		}) as typeof net.default.createConnection);

		try {
			const connected = await (client as any).openSocket();
			expect(connected).toBe(socket);
			expect(socket.off).toHaveBeenCalledWith("error", expect.any(Function));
			// Capture the removed listener and invoke it: connect-phase settle is done,
			// so it must not destroy/reject again.
			const offCall = socket.off.mock.calls.find((call) => call[0] === "error");
			expect(offCall).toBeDefined();
			const removedListener = offCall![1] as (error: Error) => void;
			removedListener(new Error("post-connect"));
