...
describe("config + docs parity", () => {
	...
	it("keeps README, SKILL, and tool help defaults aligned with config defaults", async () => {
		...
		expect(readme).toContain(`"connectTimeoutMs": ${defaults.kitty.connectTimeoutMs}`);
		expect(readme).toContain(`"killGraceMs": ${defaults.kitty.killGraceMs}`);
		expect(readme).not.toContain("transferLines");
		expect(readme).not.toContain("transferMaxChars");
