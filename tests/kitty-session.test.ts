...
function testSessionCacheDirName(sessionId: string): string {
...
describe("computeSnapshotDelta", () => {
	...
	it("handles one-line roll-off without shared trailing-newline shape", () => {
		const previous = "line1\nline2";
		const next = "line2\nline3";
		expect(computeSnapshotDelta(previous, next)).toEqual({ delta: "\nline3", rewrite: false });
	});
	...
	it("surfaces changed rewrite lines for stream monitors", () => {
		expect(computeRewriteMonitorPayload("Progress: 10%\n", "ERROR: failed\n")).toBe("ERROR: failed\n");
	});
});
...
describe("sessionCacheDirName / capLinesByMaxChars", () => {
	it("keeps distinct session ids in distinct cache dirs even when sanitized prefixes collide", async () => {
		...
		const { sessionCacheDirName } = await import("../kitty-session.js");
		expect(sessionCacheDirName("job/a")).not.toBe(sessionCacheDirName("job?a"));
		expect(sessionCacheDirName("job/a")).toContain("job_a");
		vi.doUnmock("@mariozechner/pi-coding-agent");
	});

	it("hard-caps joined line length including newlines", async () => {
		vi.resetModules();
		vi.doMock("@mariozechner/pi-coding-agent", () => ({
			getAgentDir: () => "/tmp/pi-agent-kitty-session-test",
		}));
		...
		expect(capped.lines[0]).toBe("x".repeat(10));
		vi.doUnmock("@mariozechner/pi-coding-agent");
	});
});
...
describe("KittyTerminalSession poll transient failures", () => {
	...
	it("does not mark exited when ls also fails after sustained getText outages", async () => {
		const getText = vi.fn().mockRejectedValue(new Error("rc down"));
		// Launch + initial metadata need successful ls; later probes fail.
		const healthyLs = [
			{
				id: 1,
				tabs: [
		...
		const ls = vi.fn(async () => {
			lsCalls += 1;
			// Allow launch + refreshMetadata, then fail all subsequent probes.
			if (lsCalls <= 2) return healthyLs;
		...
		const { KittyTerminalSession } = await import("../kitty-session.js");
		const session = new KittyTerminalSession({ command: "sleep 999", id: `rc-outage-${Math.random().toString(36).slice(2)}` }, {
			scrollbackLines: 5000,
			kitty: {
				version: [0, 47, 4] as [number, number, number],
				responseTimeoutMs: 5000,
		...
		await session.ready;
		// Exceed MAX_CONSECUTIVE_POLL_FAILURES while both getText and ls fail.
		for (let i = 0; i < 8; i++) {
		...
		expect(session.exited).toBe(false);
		expect(lsCalls).toBeGreaterThan(2);
		session.dispose();
	});
	...
	it("captures final kitty text before marking exit from exit file", async () => {
		...
		writeFileSync(
			`/tmp/pi-agent-kitty-session-test/cache/interactive-kitty/kitty/${testSessionCacheDirName(sessionId)}/exit-code.txt`,
			"0",
			"utf8",
		);
	...
	it("removes a stale exit file before launching a reused session id", async () => {
		...
		const sessionDir = `${agentDir}/cache/interactive-kitty/kitty/${testSessionCacheDirName(sessionId)}`;
	...
	it("pulses data listeners on full snapshot rewrite without duplicating raw stream", async () => {
		...
		// Stream monitors receive changed rewrite lines; raw append-only stream stays pre-rewrite.
		const rewritePulses = pulses.filter((p) => p.includes("different") || p.includes("totally") || p.includes("\u200c"));
		expect(rewritePulses.length).toBeGreaterThan(0);
		// Full original snapshot should not be re-appended as a single pulse after rewrite.
		expect(rewritePulses.some((p) => p === beforeRewrite)).toBe(false);
	...
	it("addExitListener replays immediately when the session already exited", async () => {
		const sessionId = `exit-replay-${Math.random().toString(36).slice(2)}`;
		const getText = vi.fn().mockResolvedValue("done\n");
		const ls = mockLsWithManagedWindow();
		...
		const { KittyTerminalSession } = await import("../kitty-session.js");
		const session = new KittyTerminalSession({ command: "true", id: sessionId }, {
			scrollbackLines: 5000,
			kitty: {
				version: [0, 47, 4] as [number, number, number],
				responseTimeoutMs: 5000,
		...
		await session.ready;
		// Simulate a fast exit that finished before interactive mode attached its waiter.
		writeFileSync(
			`/tmp/pi-agent-kitty-session-test/cache/interactive-kitty/kitty/${testSessionCacheDirName(sessionId)}/exit-code.txt`,
			"0",
			"utf8",
