...
export class KittyTerminalSession implements TerminalSession {
	...
	constructor(options: TerminalSessionOptions, config: InteractiveShellConfig, events: TerminalSessionEvents = {}) {
		...
		// Hash the full id so sanitized prefixes cannot collide across distinct sessions.
		this.sessionDir = join(getAgentDir(), "cache", "interactive-kitty", "kitty", sessionCacheDirName(sessionId));
		mkdirSync(this.sessionDir, { recursive: true });
		this.exitFile = join(this.sessionDir, "exit-code.txt");
		this.wrapperPath = join(this.sessionDir, "runner.mjs");
	...
	addExitListener(cb: (exitCode: number | null, signal?: number) => void): () => void {
		// Replay for listeners registered after a fast exit (e.g. `true`/`echo`), so
		// interactive-mode waiters do not hang forever waiting for a missed event.
		if (this._exited) {
			cb(this._exitCode, this._signal);
			return () => {};
		}
	...
	async sendKeysAsync(keys: string[]): Promise<void> {
		...
		await this.queue.enqueue(async () => {
			...
			const flushText = async () => {
				...
				textBuf = "";
				await this.client.sendText(this.windowId, payload);
			};
			const flushKeys = async () => {
				if (keyBuf.length === 0) return;
				const payload = keyBuf;
				keyBuf = [];
				await this.client.sendKeys(this.windowId, payload);
			};
	...
	async getTailLines(options: { lines: number; ansi?: boolean; maxChars?: number }): Promise<{
		...
		const capped = capLinesByMaxChars(selected, Math.max(0, Math.trunc(options.maxChars)));
		return {
			lines: capped.lines,
			totalLinesInBuffer: lines.length,
	...
	dispose(): void {
		...
		// Safe once the managed tab is closed: runner.mjs is no longer needed.
		this.cleanupSessionDir();
	}
	...
	private async launch(options: TerminalSessionOptions, sessionId: string): Promise<void> {
		try {
			...
			if (options.focus ?? this.config.kitty?.focusNewSessions ?? true) {
				// Pitfall: do NOT call this.focus() here. focus() awaits this.ready, and
				// this.ready IS the launch() promise — so focus:true (default for non-
				// background sessions) deadlocks forever: the kitty tab opens but ready
				// never resolves and the tool call hangs. Focus via RC with windowId set.
				await this.client.focusTabForWindow(this.windowId).catch(() => {});
				await this.client.focusWindow(this.windowId).catch(() => {});
			}
		} catch (error) {
			if (this.windowId > 0) {
	...
	private async poll(): Promise<void> {
		...
		try {
			...
			if (!windowPresent) {
				...
				// Successful ls that proves the window is gone is a real exit signal.
			...
			// Threshold reached: only mark exited when ls successfully proves the window is gone.
			// If ls itself throws, treat it as continued RC outage — not process exit.
			try {
				...
				const lsMessage = lsError instanceof Error ? lsError.message : String(lsError);
				console.error(`interactive-shell: kitty RC still unavailable after poll failures (not treating as process exit):`, lsMessage);
				// Soft-reset so we keep polling rather than thrashing every tick at the threshold.
				this.consecutivePollFailures = Math.max(0, MAX_CONSECUTIVE_POLL_FAILURES - 2);
			}
		} finally {
	...
	/**
	 * Strip trailing blank screen-padding rows from a stream delta (kitty's extent:all
	 * pads below the cursor) while preserving ANSI and a single trailing newline.
	 * Returns "" when the delta is entirely blank so it is not appended to rawOutput —
	 * otherwise drain sees ~rows of leading/trailing blanks around real content.
	 */
	private trimStreamDelta(delta: string): string {
		const stripped = stripVTControlCharacters(delta);
		if (stripped.trim().length === 0) return "";
		const rawLines = delta.split("\n");
		const strippedLines = stripped.split("\n");
		let pop = 0;
		while (pop < strippedLines.length && strippedLines[strippedLines.length - 1 - pop]!.trim().length === 0) pop++;
		if (pop > 0) rawLines.splice(rawLines.length - pop, pop);
	...
	private ingestSnapshot(snapshot: string): void {
		...
		if (rewrite) {
			...
			// Surface changed lines to stream monitors so in-place TUI updates still match
			// triggers (e.g. "Progress: 10%" -> "ERROR: failed") without polluting rawOutput.
			const rewritePayload = computeRewriteMonitorPayload(previous, next);
			if (rewritePayload) {
				this.notifyDataListeners(rewritePayload);
			} else if (stripVTControlCharacters(next).trim().length > 0) {
		...
		if (!delta) return;
		const trimmed = this.trimStreamDelta(delta);
		if (!trimmed) return;
		this.rawOutput += trimmed;
		this.trimStreamBuffer();
		this.notifyDataListeners(trimmed);
	}
	...
	private cleanupSessionDir(): void {
		try {
			...
			// Best-effort: cache dir may already be gone.
		}
	}
	...
	private linesFromText(sourceText: string, ansi: boolean): string[] {
		...
		// Pitfall: kitty get-text extent:all is a fixed-size screen buffer padded with
		// empty rows below the cursor. Tail/viewport "last N lines" would otherwise
		// return only blanks for short-lived commands. Drop trailing padding only.
	...
	private readExitStatus(): { exitCode: number | null; signal?: number } | undefined {
		...
		try {
			...
			// Race: exit file may disappear between existsSync and readFileSync.
			return undefined;
		}
	}
...
/**
 * Directory name under cache/interactive-kitty/kitty.
 * Hash the full id so sanitized prefixes cannot collide across distinct session ids
 * (e.g. "foo/bar" and "foo_bar" both sanitize toward the same path segment).
 */
export function sessionCacheDirName(sessionId: string): string {
	const hash = createHash("sha256").update(sessionId).digest("hex").slice(0, 16);
	const safe = sessionId.replace(/[^A-Za-z0-9_.-]/g, "_").slice(0, 48) || "session";
	return `${safe}-${hash}`;
}
...
child.on("error", (err) => {
  try { writeFileSync(${JSON.stringify(exitFile)}, "127"); } catch {}
